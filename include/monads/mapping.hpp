...
private:
  ...
  struct Currier {
    ...
    template <class... T>
    Currier(Func f, T&&... args) noexcept(noexcept(Func(std::move(f))) && noexcept(TupT(std::forward<T>(args)...)))
  ...
  private:
    ...
    static auto OpImplIface(Me&& me, NewArgs&&... new_args) {
      return OpImpl(std::forward<Me>(me), std::make_index_sequence<sizeof...(Args)>(), std::forward<NewArgs>(new_args)...);
    }
    ...
    static auto OpImpl(Me&& me, std::index_sequence<ixs...>, NewArgs&&... new_args) {
      ...
      return static_cast<FuncForward>(me.func)(std::get<ixs>(static_cast<TupForward>(me.args_copy))...,
